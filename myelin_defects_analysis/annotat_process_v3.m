...
function data = load_data()
    ...
    for i = 1:length(Files)
        ...
        if isempty(UIoutput.vesicle_diameter)
            data(i).vesicle_coord = NaN;
            data(i).N_vesicles = 0;  
        else
            data(i).vesicle_coord = UIoutput.vesicle;
            data(i).N_vesicles = numel(data(i).vesicle_coord);   
        end
        % find info about myelin abnormalities
        if isempty(UIoutput.myelin_count)
            data(i).myelin_def_coord = NaN;
            data(i).N_myelin_def = 0;
        else
            data(i).myelin_def_coord = UIoutput.myelin;
            data(i).N_myelin_def = UIoutput.myelin_count;
        end
        % find info about balloons
        if isempty(UIoutput.balloon_raio)
            data(i).balloon_coord = NaN;
            data(i).N_balloons = 0;
        else
            data(i).balloon_coord = UIoutput.balloon;
